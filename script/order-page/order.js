import { clearOrderCart, orders } from "../../data/order-data.js";
...
let deliveryChargeOfTheItems = 0;
...
orders.forEach((order) => {
  ...
  let totalPriceOfTheDay = order.totalCostCents;
  const orderId = order.id;

  //getting delivery charge first
  order.products.forEach((product) => {
    ...
    if (matchingItem) {
      ...
      deliveryChargeOfTheItems += deliveryOption.priceCents;
    }
  });
  deliveryChargeOfTheItems += deliveryChargeOfTheItems / 10;
  totalPriceOfTheDay += deliveryChargeOfTheItems - 1;
  totalPriceOfTheDay = moneyFormate(totalPriceOfTheDay);
  //adding the HTML
  orderHTML += `
    ...
    if (matchingItem) {
      ...
      orderHTML += `
            ...
            <div class="product-actions">
                ...
                <button class="track-package-button button-secondary js-track-package-button " data-product-id="${productId}">
                  Track package
                </button>
...
//managing the order array when clicking the cart link

document.querySelector(".track-order-js-header-link").addEventListener("click", () => {
  clearOrderCart();
});
document.querySelector(".track-order-js-cart-link").addEventListener("click", () => {
  clearOrderCart();
});
  //clicking the tracking button
  document.querySelectorAll(".js-track-package-button").forEach((button)=>{
    button.addEventListener("click",()=>{
      const productId=button.dataset.productId;
      window.location.href=`tracking.html?productId=${productId}`;
    });
  });